...
const app = express();
app.use(
  session({ secret: "miappsegura", resave: false, saveUninitialized: true })
);
const port = 3000;
...
app.post("/", (req, res) => {
  ...
  request(
    `http://${host}/codigos?api_key=${api.key}`,
...
app.get("/codigos", (req, res) => {
  ...
  if (req.query.api_key == api.key) res.json(codigos);
  else res.status(401).send("Sin autorización");
});
